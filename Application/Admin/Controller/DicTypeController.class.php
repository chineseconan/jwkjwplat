...
class DicTypeController extends BaseController {
    ...
    public function index(){
        $this->addLog('','用户访问日志','','访问字典类型管理页面','成功');
        $this->display();
    }
    ...
    public function add(){
        ...
        $this->addLog('','用户访问日志','','访问字典类型添加、编辑页面','成功');
        $this->display();
    }
    ...
    public function addDicType(){
        ...
        if(empty($id)){
            ...
            $res = $model->add($data);
            if(empty($res)){
                $this->addLog('dic_type', '对象修改日志', 'add', '添加字典类型=>'.$data['type_name']. '，失败','失败');
                exit(makeStandResult(-1,'添加失败'));
            }else{
                $this->addLog('dic_type', '对象修改日志', 'add', '添加字典类型=>'.$data['type_name']. '，成功','成功');
            ...
            if(empty($res)){
                $this->addLog('dic_type', '对象修改日志', 'update', '修改字典类型=>'.$typeName['type_name']. '，失败','失败');
                exit(makeStandResult(-1,'修改失败'));
            }else{
                $this->addLog('dic_type', '对象修改日志', 'update', '修改字典类型=>'.$typeName['type_name']. '，成功','成功');
    ...
    public function delDicType(){
        ...
        if(empty($res)){
            $this->addLog('dic_type', '对象修改日志', 'delete', '删除字典类型=>'.$names. '，失败','失败');
            exit(makeStandResult(-1,'删除失败'));
        }else{
            $this->addLog('dic_type', '对象修改日志', 'delete', '删除字典类型=>'.$names. '，成功','成功');
