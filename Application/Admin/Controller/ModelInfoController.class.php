...
//        $this->assign('dictionaryType', $dicType);
        $model = M('modelinfo');
        $modelList = $model->field('mi_id,mi_name,mi_pid')->where("mi_id!='%s' and (mi_type is null or mi_type='功能类型')")->select();
        $this->assign('modellist', $modelList);
//        $this->addLog('','用户访问日志','','访问模块管理','成功');
        $this->display();
    }
    ...
    public function getData(){
        ...
        if(!empty($miName))
        {
            //$this->addLog('','用户访问日志','','访问模块管理,查询模块名称关键字:'.$miName,'成功');
...
//        $this->addLog('','对象修改日志','','导出模块列表','成功');
    ...
    public function add(){
        ...
        if(!empty($id)){
            ...
            }else
            {
                ...
                $modelList = $model->field('mi_id,mi_name,mi_pid')->where("mi_id!='%s' and (mi_type is null or mi_type='功能类型')", $id)->select();
                //$this->addLog('','用户访问日志','','访问'.$data['mi_name'].'模块修改','成功');
                $this->assign('data', $data);
            }
        }
        //$this->addLog('','用户访问日志','','访问模块添加','成功');
    ...
    public function addModelInfo(){
        ...
        if(empty($id)){
            ...
            if(!empty($tem))
            {
//                $this->addLog('modelinfo','三员操作日志','add','新增模块'.$data['mi_name'],'失败');
...
//                $this->addLog('modelinfo','三员操作日志','add','新增模块'.$data['mi_name'],'成功');
            ...
            {
//                $this->addLog('modelinfo','三员操作日志','update','修改模块'.$data['mi_name'],'失败');
                exit(makeStandResult(-1,'模块已存在'));
            }
...
//                $this->addLog('modelinfo','三员操作日志','update','修改模块'.$data['mi_name'],'成功');
    ...
    public function delModelInfo(){
        ...
        foreach($id as $key=>$val)
        {
            ...
            if(!empty($tem))
            {
//                $this->addLog('modelinfo','三员操作日志','delete','删除模块'.$tem['mi_name'],'成功');
            }
        }
        $roleauth->where("ra_miid in ($idStr)")->delete();
        $res = $model -> where("mi_id in ($idStr)")->delete();
        if(empty($res)){
//            $this->addLog('modelinfo','三员操作日志','delete','删除模块','失败');
            exit(makeStandResult(-1,'删除失败'));
        }else{
//            $this->addLog('modelinfo','三员操作日志','delete','删除模块','成功');
